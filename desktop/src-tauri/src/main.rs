// Prevents additional console window on Windows in release, DO NOT REMOVE!!
#![cfg_attr(not(debug_assertions), windows_subsystem = "windows")]

mod cleaner;
mod cli;
mod cmd;
mod config;
mod panic_hook;

#[cfg(feature = "server")]
mod server;

mod setup;
mod utils;
use tauri::{Emitter, Manager};
mod logging;

#[cfg(target_os = "macos")]
mod dock;

#[cfg(all(any(target_arch = "x86", target_arch = "x86_64"), target_os = "windows"))]
mod x86_features;

#[cfg(windows)]
mod custom_protocol;

#[cfg(windows)]
mod gpu_preference;

#[cfg(target_os = "macos")]
mod screen_capture_kit;

use eyre::{eyre, Result};
use tauri_plugin_global_shortcut::{Code, Modifiers, Shortcut, GlobalShortcutExt};
use tauri_plugin_window_state::StateFlags;
#[cfg(target_os = "macos")]
use tauri_plugin_dialog::{DialogExt, MessageDialogButtons, MessageDialogKind};

use utils::LogError;

fn main() -> Result<()> {
    // Attach console in Windows:
    #[cfg(all(windows, not(debug_assertions)))]
    cli::attach_console();

    tauri::Builder::default()
        .plugin(tauri_plugin_http::init())
        .plugin(tauri_plugin_clipboard_manager::init())
        .plugin(tauri_plugin_single_instance::init(|app, argv, cwd| {
            tracing::debug!("{}, {argv:?}, {cwd}", app.package_info().name);
            if let Some(webview) = app.get_webview_window("main") {
                webview.set_focus().map_err(|e| eyre!("{:?}", e)).log_error();
            }
            app.emit("single-instance", argv).map_err(|e| eyre!("{:?}", e)).log_error();
        }))
        .plugin(tauri_plugin_global_shortcut::Builder::new().build())
        .setup(|app| {
            setup::setup(app)?;
            let app_handle = app.handle().clone();
            
            // Register global shortcut after setup
            // Using Option+Shift+R on macOS and Alt+Shift+R on Windows/Linux to avoid conflicts
            #[cfg(target_os = "macos")]
            let modifiers = Modifiers::ALT | Modifiers::SHIFT;
            #[cfg(not(target_os = "macos"))]
            let modifiers = Modifiers::ALT | Modifiers::SHIFT;
            
            let shortcut = Shortcut::new(Some(modifiers), Code::KeyR);
            eprintln!("🔧 Attempting to register global shortcut: modifiers={:?} (bits: {:b}), code={:?}", modifiers, modifiers.bits(), Code::KeyR);
            tracing::info!("Attempting to register global shortcut: modifiers={:?} (bits: {:b}), code={:?}", modifiers, modifiers.bits(), Code::KeyR);
            
            // Use on_shortcut which should register and set up handler in one call
            // Add debouncing with 500ms cooldown (like VoiceInk) to prevent multiple rapid triggers
            use std::sync::atomic::{AtomicBool, AtomicU64, Ordering};
            use std::sync::Arc;
            use std::time::{SystemTime, UNIX_EPOCH};
            
            let shortcut_debounce = Arc::new(AtomicBool::new(false));
            let shortcut_debounce_clone = shortcut_debounce.clone();
            let last_shortcut_time = Arc::new(AtomicU64::new(0));
            let last_shortcut_time_clone = last_shortcut_time.clone();
            
            match app_handle.global_shortcut().on_shortcut(shortcut, move |app, _shortcut, _event| {
                // Check 500ms cooldown first (like VoiceInk)
                let now = SystemTime::now()
                    .duration_since(UNIX_EPOCH)
                    .map(|d| d.as_millis() as u64)
                    .unwrap_or(0);
                let last_time = last_shortcut_time_clone.load(Ordering::SeqCst);
                if now.saturating_sub(last_time) < 500 {
                    tracing::debug!("Shortcut cooldown active ({}ms since last), ignoring", now.saturating_sub(last_time));
                    return;
                }
                
                // Debounce: ignore if already processing
                if shortcut_debounce_clone
                    .compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst)
                    .is_err()
                {
                    tracing::debug!("Shortcut already being processed, ignoring duplicate");
                    return;
                }
                
                // Update last shortcut time
                last_shortcut_time_clone.store(now, Ordering::SeqCst);
                
                eprintln!("🔥🔥🔥 Global shortcut pressed! Toggling recording...");
                tracing::info!("🔥🔥🔥 Global shortcut pressed! Toggling recording...");
                let app_handle = app.app_handle().clone();
                let debounce = shortcut_debounce_clone.clone();
                
                tauri::async_runtime::spawn(async move {
                    tracing::info!("Executing toggle_record_internal...");
                    match cmd::audio::toggle_record_internal(app_handle).await {
                        Ok(_) => {
                            tracing::info!("✓ Successfully toggled recording");
                        }
                        Err(e) => {
                            tracing::error!("✗ Failed to toggle recording: {:?}", e);
                        }
                    }
                    // Reset debounce after 500ms cooldown
                    tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
                    debounce.store(false, Ordering::SeqCst);
                });
            }) {
                Ok(_) => {
                    eprintln!("✅ Successfully registered and set up global shortcut: Option+Shift+R (macOS) / Alt+Shift+R (Windows/Linux)");
                    tracing::info!("✓ Successfully registered and set up global shortcut: Option+Shift+R (macOS) / Alt+Shift+R (Windows/Linux)");
                }
                Err(e) => {
                    eprintln!("❌ Failed to register global shortcut (modifiers: {:?}, code: {:?}): {:?}. Global shortcut will not be available.", modifiers, Code::KeyR, e);
                    tracing::error!("✗ Failed to register global shortcut (modifiers: {:?}, code: {:?}): {:?}. Global shortcut will not be available.", modifiers, Code::KeyR, e);
                    
                    // On macOS, check if it's due to missing permissions and show dialog
                    #[cfg(target_os = "macos")]
                    {
                        if !utils::has_accessibility_permissions() {
                            tracing::error!("   This is likely due to missing accessibility permissions. Please grant permissions in System Preferences.");
                            
                            // Show a dialog to inform the user
                            let app_handle_clone = app_handle.clone();
                            tauri::async_runtime::spawn(async move {
                                // Wait a bit for the window to be ready
                                tokio::time::sleep(tokio::time::Duration::from_millis(1000)).await;
                                
                                if let Some(_window) = app_handle_clone.get_webview_window("main") {
                                    let _ = app_handle_clone.dialog().message(
                                        "Anamedi requires accessibility permissions to use global keyboard shortcuts (Option+Shift+R).\n\nPlease grant accessibility permissions in:\nSystem Preferences > Security & Privacy > Accessibility\n\nThen restart the app."
                                    )
                                    .kind(MessageDialogKind::Warning)
                                    .title("Accessibility Permissions Required")
                                    .buttons(MessageDialogButtons::Ok)
                                    .show(|_| {});
                                }
                            });
                        }
                    }
                }
            }
            
            // Check accessibility permissions on macOS and log status
            #[cfg(target_os = "macos")]
            {
                if utils::has_accessibility_permissions() {
                    eprintln!("✅ Accessibility permissions granted");
                    tracing::info!("✓ Accessibility permissions granted");
                } else {
                    eprintln!("⚠️  Accessibility permissions NOT granted. Global shortcuts may not work.");
                    tracing::warn!("⚠️  Accessibility permissions not granted. Global shortcuts may not work.");
                }
            }
            
            Ok(())
        })
        .plugin(
            tauri_plugin_window_state::Builder::default()
                .with_state_flags(!StateFlags::VISIBLE)
                .build(),
        )
        .plugin(tauri_plugin_store::Builder::default().build())
        .plugin(tauri_plugin_deep_link::init())
        .plugin(tauri_plugin_fs::init())
        .plugin(tauri_plugin_os::init())
        .plugin(tauri_plugin_dialog::init())
        .plugin(tauri_plugin_updater::Builder::default().build())
        .plugin(tauri_plugin_process::init())
        .plugin(tauri_plugin_shell::init())
        .plugin(tauri_plugin_keepawake::init())
        .invoke_handler(tauri::generate_handler![
            cmd::download_file,
            cmd::get_cargo_features,
            cmd::transcribe,
            cmd::glob_files,
            cmd::download_model,
            cmd::load_model,
            cmd::get_commit_hash,
            cmd::get_cuda_version,
            cmd::get_rocm_version,
            cmd::is_avx2_enabled,
            cmd::is_online,
            cmd::get_path_dst,
            cmd::get_logs,
            cmd::open_path,
            cmd::get_x86_features,
            cmd::get_save_path,
            cmd::get_argv,
            cmd::audio::get_audio_devices,
            cmd::audio::start_record,
            cmd::audio::toggle_record,
            cmd::audio::set_ignore_cursor_events,
            cmd::get_models_folder,
            cmd::is_portable,
            cmd::check_vulkan,
            cmd::get_logs_folder,
            cmd::show_log_path,
            cmd::show_temp_path,
            cmd::get_ffmpeg_path,
            cmd::is_crashed_recently,
            cmd::rename_crash_file,
            cmd::simulate_paste,
            cmd::restore_focus_to_previous_app,
            cmd::register_global_shortcut,
            cmd::unregister_all_shortcuts,
            #[cfg(windows)]
            cmd::set_high_gpu_preference
        ])
        .run(tauri::generate_context!())
        .expect("error while running tauri application");
    Ok(())
}
