...
function Paste({ text }: { text: string }) {
	const { t } = useTranslation()
	const [info, setInfo] = useState(t('common.paste'))
	...
	async function onPaste() {
		try {
			...
			setInfo(t('common.pasted'))
			setTimeout(() => setInfo(t('common.paste')), 1000)
		} catch (error) {
			console.error('Failed to paste:', error)
			setInfo(t('common.paste-error'))
			setTimeout(() => setInfo(t('common.paste')), 2000)
		}
	}
	return (
		<div className="tooltip tooltip-bottom" data-tip={info}>
			<button className="btn btn-square btn-md" onMouseDown={onPaste}>
				<svg
